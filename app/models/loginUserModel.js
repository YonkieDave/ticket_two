...
module.exports.userExist = async (usr) => {
     ...
     let result = await sequelize.query(`SELECT * FROM users WHERE user_email = '${usr.user}' AND user_pass = '${usr.pass}'`);
     //console.log(`resultado --->  ${JSON.stringify(result[0][0].rol)}   Logitud ${result[0].length}`)   
     if (result[0].length == 0) {
           return false;
        }else{
            let  resp = [JSON.stringify(result[0][0].user_id), JSON.stringify(result[0][0].user_name)]; 
           return (true, resp );
